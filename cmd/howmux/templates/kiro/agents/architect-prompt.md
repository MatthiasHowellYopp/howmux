...
**Example from PR #20**: The spec for "Rename project from howmux to something-else" did not call out `.gitignore` or CI workflows as part of the surface. Builder renamed the code but not `.gitignore` → committed runtime artifacts slipped through because `.gitignore` still pointed at the old `.howmux/` paths.
...
1. All references to `old-name` in `.go` files renamed to `howmux`
2. All references to `old-name` in test files (`*_test.go`) renamed to `howmux`
3. All references to `old-name` in `README.md` and docs files renamed to `howmux`
4. `.gitignore` updated: all `.old-name/` paths renamed to `.howmux/`
5. `Taskfile.yml` updated: all task names and paths referencing `old-name` renamed to `howmux`
6. `.github/workflows/*.yml` updated: all job names, artifact names, and paths referencing `old-name` renamed to `howmux`
...
10. Repo-wide search for `old-name` returns only justified/intentional matches (documented)
```
...
1. Rename project from old-name to howmux
...
1. All references to `old-name` in source, tests, docs, config, and CI renamed to `new-name`
   - **Verification**: `grep -rn "old-name" --include="*.go" --include="*.md" --include="*.yaml" --include="*.yml" --include=".gitignore" .` returns only justified matches (documented in PR body)

2. Config files updated to new name:
   - **Verification**: `grep -rn "old-name" .gitignore .github/workflows/ Taskfile.yml` returns zero matches
