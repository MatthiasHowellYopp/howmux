...
**Example from PR #20**: The spec for "Rename project from howmux to howmux" did not call out `.gitignore` or CI workflows as part of the surface. Builder renamed the code but not `.gitignore` → committed runtime artifacts slipped through because `.gitignore` still pointed at the old `.howmux/` paths.
...
- Scripts (`.howmux/scripts/*.sh`)
- Themes (`.howmux/themes/*.yaml`)
...
1. All references to `howmux` in `.go` files renamed to `howmux`
2. All references to `howmux` in test files (`*_test.go`) renamed to `howmux`
3. All references to `howmux` in `README.md` and docs files renamed to `howmux`
4. `.gitignore` updated: all `.howmux/` paths renamed to `.howmux/`
5. `Taskfile.yml` updated: all task names and paths referencing `howmux` renamed to `howmux`
6. `.github/workflows/*.yml` updated: all job names, artifact names, and paths referencing `howmux` renamed to `howmux`
7. Agent configs (`.kiro/agents/*.json`, `.kiro/agents/*.md`) renamed and updated
8. Template files under `cmd/howmux/templates/` renamed and synced
9. Environment variables: `HOWMUX_*` renamed to `HOWMUX_*` in both writer and reader
10. Repo-wide search for `howmux` returns only justified/intentional matches (documented)
```
...
1. Rename project from howmux to howmux
...
```markdown
5. Environment variable `HOWMUX_WATCHER_PID` renamed to `HOWMUX_WATCHER_PID`:
   - Writer in `internal/agent/manager.go` uses new name
   - Reader in `internal/hotkey/detector.go` uses new name
   - Verify via `grep -rn "HOWMUX_WATCHER_PID"` returns zero matches
```
...
1. All references to `howmux` in source, tests, docs, config, and CI renamed to `howmux`
   - **Verification**: `grep -rn "howmux" --include="*.go" --include="*.md" --include="*.yaml" --include="*.yml" --include=".gitignore" .` returns only justified matches (documented in PR body)

2. Config files updated to new name:
   - **Verification**: `grep -rn "howmux" .gitignore .github/workflows/ Taskfile.yml` returns zero matches

3. Environment variables renamed in writer and reader:
   - **Verification**: `grep -rn "HOWMUX" --include="*.go" .` returns zero matches
```
...
1. **Source code**: All references to `howmux` in `.go` files renamed to `howmux`
   - Package paths: `github.com/jbrinkman/howmux` → `github.com/matthiashowellyopp/howmux`
   - Struct/type names, if any, referencing the old project name

2. **Test files**: All references to `howmux` in `*_test.go` files renamed to `howmux`

3. **Documentation**: All references to `howmux` in `README.md`, docs, and comments renamed to `howmux`
   - User-facing strings (About dialog, help text) updated to "Howmux"

4. **Configuration files**:
   - `.gitignore`: All `.howmux/` paths renamed to `.howmux/`
   - `Taskfile.yml`: Task names and paths referencing `howmux` renamed to `howmux`
   - `.releaserc.json`: Artifact/release names updated

5. **CI/Build workflows**:
   - `.github/workflows/*.yml`: Job names, artifact names, paths referencing `howmux` renamed to `howmux`
   - Build scripts: Any hardcoded paths or names updated

6. **Template-synced files**:
   - Agent configs under `cmd/howmux/templates/kiro/agents/` renamed and synced
   - Scripts under `cmd/howmux/templates/howmux/scripts/` synced

7. **Environment variables**: All `HOWMUX_*` env vars renamed to `HOWMUX_*`
   - `HOWMUX_WATCHER_PID` → `HOWMUX_WATCHER_PID` in both writer (`internal/agent/manager.go`) and reader (`internal/hotkey/detector.go`)
   - `HOWMUX_EVAL_TIMEOUT` → `HOWMUX_EVAL_TIMEOUT` in `internal/eval/runner.go`
   - Verify: `grep -rn "HOWMUX" --include="*.go" .` returns zero matches

8. **Completeness check**: Repo-wide search for `howmux` returns only justified/intentional matches
   - **Verification**: `grep -rn "howmux" --include="*.go" --include="*.md" --include="*.yaml" . | wc -l` → document any remaining matches in PR body as justified exceptions
```

**What actually happened** (spec did not enumerate config files / env-var writer+reader / templates):
- ~24-38 stray `howmux` references in `.go` files
- `.gitignore` still had `.howmux/` paths
