...
func TestHandleDecide_ValidSpoolPath_WriterSuccess(t *testing.T) {
	...
	m := newDecideTestModel()
	addReviewsTabWithRecord(m, review.Record{Repo: "owner/repo", PR: 7, SpoolPath: "/tmp/spool.md"})
...
func TestHandleDecide_WorksWhenDifferentTabActive(t *testing.T) {
	...
	m := newDecideTestModel()
	addReviewsTabWithRecord(m, review.Record{Repo: "owner/repo", PR: 13, SpoolPath: "/tmp/spool.md"})
