...
// TestReviewsTabFinalizePreflightStatusBlock verifies SetFinalizePreflightState
// drives the status block prepended by View() (styled) and CopyableContent()
// (plain), and that the block is fully replaced (not appended) once the state
// clears back to OK — the copy/view parity plus "no stale text lingers"
// guarantee AC4/Task 4 require.
func TestReviewsTabFinalizePreflightStatusBlock(t *testing.T) {
	store := &fakeReviewStore{records: nil}
	rt := NewReviewsTab("reviews", store, testReviewsStyles())

	rt.SetFinalizePreflightState(finalizePreflightFailed, "boom")

	view := rt.View()
	if !strings.Contains(view, "boom") {
		t.Errorf("expected View() to contain preflight error text %q, got %q", "boom", view)
	}
	if !strings.Contains(view, "press F to retry") {
		t.Errorf("expected View() to contain the retry hint, got %q", view)
	}

	content := rt.CopyableContent()
	if !strings.Contains(content, "boom") {
		t.Errorf("expected CopyableContent() to contain preflight error text %q, got %q", "boom", content)
	}

	rt.SetFinalizePreflightState(finalizePreflightOK, "")

	view = rt.View()
	if strings.Contains(view, "boom") {
		t.Errorf("expected View() to no longer contain the old preflight error text, got %q", view)
	}
	if strings.Contains(view, "press F to retry") {
		t.Errorf("expected View() to no longer contain the retry hint after clearing, got %q", view)
	}

	content = rt.CopyableContent()
	if strings.Contains(content, "boom") {
		t.Errorf("expected CopyableContent() to no longer contain the old preflight error text, got %q", content)
	}
}

// TestReviewsTabFinalizePreflightUnknownAndOKRenderNoExtraBlock verifies
// Unknown and OK states render no additional block beyond the tab's normal
// content — only Failed shows the warning.
func TestReviewsTabFinalizePreflightUnknownAndOKRenderNoExtraBlock(t *testing.T) {
	store := &fakeReviewStore{records: nil}

	for _, state := range []finalizePreflightState{finalizePreflightUnknown, finalizePreflightOK} {
		rt := NewReviewsTab("reviews", store, testReviewsStyles())
		rt.SetFinalizePreflightState(state, "")

		view := rt.View()
		if strings.Contains(view, "Finalize gate unavailable") {
			t.Errorf("state %v: expected no warning block, got %q", state, view)
		}

		content := rt.CopyableContent()
		if strings.Contains(content, "Finalize gate unavailable") {
			t.Errorf("state %v: expected no warning block in CopyableContent, got %q", state, content)
		}
	}
}

// TestReviewsTabFinalizePreflightViewCopyParity extends
// TestReviewsTabViewCopyDrift's ANSI-stripped View()/CopyableContent()
// equality guarantee to the failed-preflight status block, so the block
// itself is never a source of drift between what's displayed and what Ctrl+Y
// copies.
func TestReviewsTabFinalizePreflightViewCopyParity(t *testing.T) {
	ansi := regexp.MustCompile(`\x1b\[[0-9;]*m`)
	strip := func(s string) string { return ansi.ReplaceAllString(s, "") }
	...
	gotCopy := rt.CopyableContent()
	if gotView != gotCopy {
		t.Errorf("View() (ANSI-stripped) and CopyableContent() drifted with a failed preflight:\nview: %q\ncopy: %q", gotView, gotCopy)
	}
}

// TestReviewsTabCapitalFKeyEmitsFinalizeRetryPreflightMsg verifies the "F"
// key (capital) returns a tea.Cmd which, when invoked, produces a
// finalizeRetryPreflightMsg — following the same message-emission pattern as
// p/r/R/d above.
func TestReviewsTabCapitalFKeyEmitsFinalizeRetryPreflightMsg(t *testing.T) {
	store := &fakeReviewStore{records: nil}
	rt := NewReviewsTab("reviews", store, testReviewsStyles())

	tab, cmd := rt.Update(decideKeyMsg('F'))
	rt = tab.(*ReviewsTab)
	if cmd == nil {
		t.Fatal("expected a non-nil tea.Cmd from Update on 'F'")
	}

	msg := cmd()
	if _, ok := msg.(finalizeRetryPreflightMsg); !ok {
		t.Fatalf("expected finalizeRetryPreflightMsg, got %T (%v)", msg, msg)
	}
}
