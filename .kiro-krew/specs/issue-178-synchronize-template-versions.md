...
Create a systematic synchronization process that treats live versions (`.howmux/`, `.kiro/`) as the authoritative source and updates all template files to match. The approach involves:
...
4. **Verification System**: Ensure `howmux init` produces identical structure to live setup

## Relevant Files

### Template Files (Source)
- `./cmd/howmux/templates/howmux/themes/*.yaml` - Theme configurations
- `./cmd/howmux/templates/kiro/agents/*.json` - Agent configurations  
- `./cmd/howmux/templates/kiro/agents/*-prompt.md` - Agent prompt files
- `./cmd/howmux/templates/howmux/evals/rubrics/*.yaml` - Evaluation rubrics
- `./cmd/howmux/templates/howmux/evals/cases/` - Evaluation test cases
- `./cmd/howmux/templates/howmux/evals/fixtures/` - Missing directory to create

### Live Files (Target/Authority)
- `./.howmux/themes/*.yaml` - Live theme configurations
- `./.kiro/agents/*.json` - Live agent configurations
- `./.kiro/agents/*-prompt.md` - Live agent prompts
- `./.howmux/evals/rubrics/*.yaml` - Live evaluation rubrics  
- `./.howmux/evals/cases/` - Live evaluation cases
- `./.howmux/evals/fixtures/` - Live fixtures to copy

### Implementation Files
- `./internal/templates/extract.go` - Template extraction logic
- `./cmd/howmux/main.go` - Embedded templates
...
2. **Validator Agent**: Verifies that post-sync `howmux init` produces correct results
...
- Handles both `.howmux/` and `.kiro/` directory synchronization

### Task 2: Synchronize Theme Configurations
**Acceptance Criteria:**
- Update all theme YAML files in `./cmd/howmux/templates/howmux/themes/`
- Add missing fields: `agent_success`, `agent_fail` color properties
- Ensure exact match with live versions in `./.howmux/themes/`
...
**Acceptance Criteria:**
- Update all rubric YAML files to match `./.howmux/evals/rubrics/`
- Sync evaluation test cases from `./.howmux/evals/cases/`
...
**Acceptance Criteria:**
- Create `./cmd/howmux/templates/howmux/evals/fixtures/` directory
- Copy all fixture files from `./.howmux/evals/fixtures/`:
  - `codebase-context.md`
...
**Acceptance Criteria:**
- Test `howmux init` in fresh directory produces identical structure to live setup
- Compare generated `.howmux/` and `.kiro/` directories with current live versions
...
- Test that `howmux update` overwrites with latest templates
...
mkdir /tmp/howmux-test
cd /tmp/howmux-test
howmux init

# Compare structures 
diff -r .howmux /path/to/live/.howmux --exclude=specs --exclude=retries --exclude=artifacts
diff -r .kiro /path/to/live/.kiro --exclude=skills

# Verify specific missing elements
ls .howmux/evals/fixtures/
grep "agent_success\|agent_fail" .howmux/themes/*.yaml  
...
go run cmd/howmux/main.go init 2>&1 | grep fixtures
```

## Technical Notes

- Live versions in `.howmux/` and `.kiro/` are the authoritative source - never modify them
...
- Ensure no breaking changes to existing `howmux init` behavior